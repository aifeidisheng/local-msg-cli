...
默认策略文件必须与代码内置摘要一致。使用自定义策略时，通过 `WECHAT_DECRYPT_POLICY_FILE` 固定策略路径，并通过 `WECHAT_DECRYPT_POLICY_SHA256` 从 MCP 宿主环境注入摘要；这两个值不应写入 Agent 可修改的项目配置。
