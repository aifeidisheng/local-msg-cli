...
本项目依赖本机微信客户端的数据结构、数据库布局和进程内密钥形态。不同版本可能存在行为差异，
在未验证版本上继续执行会带来账号风险、解密失败和数据结构不兼容风险。因此，只有命中指定版本区间时才允许执行。
...
Gitee 可以作为优先的公开候选渠道，但不是自动信任的镜像；只有版本、文件大小和已
公布的 SHA-256 都一致时，才可以把镜像和原来源关联起来。
